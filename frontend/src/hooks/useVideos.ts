...
// ¬»ѕ–ј¬Ћ≈ЌЌя: “епер ≥мпортуЇмо VideoDetail напр€му, а не через псевдон≥м
import type { VideoPreview, VideoComment, VideoDetail } from "@api/types";
import { useSearchParams } from "react-router-dom";


// ¬»ѕ–ј¬Ћ≈ЌЌя: ¬икористовуЇмо VideoPreview €к базу дл€ прев'ю з≥ стовпчика "Up Next"
type VideoPreviewWithTime = VideoPreview & {
    timeAgo: string;
};

// “ип дл€ React.Dispatch
...
export function useVideo(): UseVideoResult {
    ...
    const metaDataText = useMemo(() => {
        ...
        return `${viewCountText} views ${video.timeAgo ? 'Х ' + video.timeAgo : ''}`;
    }, [video, formatViews]);
    ...
    const fetchVideo = useCallback(async () => {
        ...
        try {
            ...
            // —творюЇмо об'Їкт VideoDetail, мапуючи пол€ з API (Video)
            setVideo({
                id: data.id || "",
                title: data.title || "",
