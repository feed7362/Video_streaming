...
export const registerUser = (username: string, password: string): Promise<string> =>
    clientApi.post<{ token: string }>("/auth/register", { username, password }).then(res => {
        localStorage.setItem("token", res.data.token);
        return res.data.token;
    });

export const loginUser = (username: string, password: string): Promise<string> =>
    clientApi.post<{ token: string }>("/auth/login", { username, password }).then(res => {
        localStorage.setItem("token", res.data.token);
        return res.data.token;
    });
...
export const refreshUsersAvatar = (username: string): Promise<string> =>
    clientApi.post<{ avatarUrl: string }>(`/auth/users/${username}/refresh-avatar`).then(res => res.data.avatarUrl);
...
export const checkUserExists = async (username: string, email: string): Promise<{ usernameExists: boolean, emailExists: boolean }> => {
    return clientApi.post("/auth/check-user", { username, email }).then(res => res.data);
};
